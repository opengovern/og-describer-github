...
// Reuse FinalRepoDetail and related structs from your "current code" snippet
type License struct {
	Key    string `json:"key"`
...
func NewResilientBridgeClient(apiToken string) *resilientbridge.ResilientBridge {
...
func fetchOrgRepos(sdk *resilientbridge.ResilientBridge, org string, maxResults int) ([]RepoDetail, error) {
	var allRepos []RepoDetail
	perPage := 100
	page := 1
...
func fetchPrivateVulnerabilityReporting(sdk *resilientbridge.ResilientBridge, owner, repo string) (bool, error) {
	req := &resilientbridge.NormalizedRequest{
		Method:   "GET",
		Endpoint: fmt.Sprintf("/repos/%s/%s/private-vulnerability-reporting", owner, repo),
		Headers:  map[string]string{"Accept": "application/vnd.github+json"},
	}
...
func countItemsFromEndpoint(sdk *resilientbridge.ResilientBridge, endpoint string) (int, error) {
	...
	if resp.StatusCode == 409 {
		// Repository might be empty, return 0
		return 0, nil
	}
	...
	if linkHeader == "" {
		// No pagination
		if len(resp.Data) > 2 { // check if it's a non-empty JSON array
			var items []interface{}
			if err := json.Unmarshal(resp.Data, &items); err != nil {
				// Not an array, might be a single object
				return 1, nil
			}
...
func transformToFinalRepoDetail(detail *RepoDetail) *FinalRepoDetail {
	...
	return &FinalRepoDetail{
		...
		SecuritySettings: SecuritySettings{
			VulnerabilityAlertsEnabled:               detail.SecurityAndAnalysis != nil && detail.SecurityAndAnalysis.DependabotSecurityUpdates != nil && detail.SecurityAndAnalysis.DependabotSecurityUpdates.Status == "enabled",
			SecretScanningEnabled:                    detail.SecurityAndAnalysis != nil && detail.SecurityAndAnalysis.SecretScanning != nil && detail.SecurityAndAnalysis.SecretScanning.Status == "enabled",
			SecretScanningPushProtectionEnabled:      detail.SecurityAndAnalysis != nil && detail.SecurityAndAnalysis.SecretScanningPushProtection != nil && detail.SecurityAndAnalysis.SecretScanningPushProtection.Status == "enabled",
			DependabotSecurityUpdatesEnabled:         detail.SecurityAndAnalysis != nil && detail.SecurityAndAnalysis.DependabotSecurityUpdates != nil && detail.SecurityAndAnalysis.DependabotSecurityUpdates.Status == "enabled",
			SecretScanningNonProviderPatternsEnabled: detail.SecurityAndAnalysis != nil && detail.SecurityAndAnalysis.SecretScanningNonProviderPatterns != nil && detail.SecurityAndAnalysis.SecretScanningNonProviderPatterns.Status == "enabled",
			SecretScanningValidityChecksEnabled:      detail.SecurityAndAnalysis != nil && detail.SecurityAndAnalysis.SecretScanningValidityChecks != nil && detail.SecurityAndAnalysis.SecretScanningValidityChecks.Status == "enabled",
			PrivateVulnerabilityReportingEnabled:     false, // set later
		},
		RepoURLs: RepoURLs{
...
func GetRepository(ctx context.Context, githubClient GitHubClient, organizationName string, repositoryName string, resourceID string, stream *models.StreamSender) (*models.Resource, error) {
	sdk := githubClient.SDK
	if sdk == nil {
		return nil, fmt.Errorf("no resilientbridge client provided")
	}

	repoDetail, err := fetchRepoDetails(sdk, organizationName, repositoryName)
	if err != nil {
		return nil, err
	}
	...
	langs, err := fetchLanguages(sdk, organizationName, repositoryName)
	if err == nil {
		if len(langs) == 0 {
	...
	if err := enrichRepoMetrics(sdk, organizationName, repositoryName, finalDetail); err != nil {
		// log error if needed
	}

	pvrEnabled, err := fetchPrivateVulnerabilityReporting(sdk, organizationName, repositoryName)
	if err == nil {
		finalDetail.SecuritySettings.PrivateVulnerabilityReportingEnabled = pvrEnabled
	}

	resource := &models.Resource{
		ID:   fmt.Sprintf("%d", finalDetail.GitHubRepoID),
...
// GetRepositoryList fetches a list of repositories for an organization.
// Adjusted signature to match what DescribeByGithub expects:
// func(ctx context.Context, githubClient GitHubClient, organizationName string, stream *models.StreamSender) ([]models.Resource, error)
func GetRepositoryList(ctx context.Context, githubClient GitHubClient, organizationName string, stream *models.StreamSender) ([]models.Resource, error) {
	...
	for _, r := range allRepos {
		finalDetail := transformToFinalRepoDetail(&r)

		langs, err := fetchLanguages(sdk, organizationName, r.Name)
		if err == nil {
			if len(langs) == 0 {
				finalDetail.Language = nil
			} else {
		...
		err = enrichRepoMetrics(sdk, organizationName, r.Name, finalDetail)
		if err != nil {
			// log error if needed
		}

		pvrEnabled, err := fetchPrivateVulnerabilityReporting(sdk, organizationName, r.Name)
		if err == nil {
			finalDetail.SecuritySettings.PrivateVulnerabilityReportingEnabled = pvrEnabled
		}
